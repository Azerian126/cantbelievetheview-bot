...
// Traduce un título/caption ya elegido (sugerido por Grok o escrito a mano)
// al inglés — se llama sola al confirmar la subida, sin agregar un paso más
// al flujo. Si falla, quien llama simplemente no guarda captionEn y el sitio
// cae al texto en español también en la versión inglesa (mejor eso que
// trabar la subida por un problema de traducción).
async function translateCaption(text) {
  const prompt =
    `Traducí este título de foto de una galería de fotografía de viajes al inglés, manteniendo el mismo ` +
    `tono corto y evocador (no literal palabra por palabra si suena mejor natural). Devolvé SOLO la ` +
    `traducción, sin comillas ni explicaciones:\n\n${text}`;
  const result = await askGrok(prompt);
  return stripDecoration(result.split('\n')[0]);
}

module.exports = { suggestCaptions, suggestCountryIntro, translateCaption };
