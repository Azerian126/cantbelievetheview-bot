...
async function askGrok(input) {
  ...
  // costInUsdTicks viene en unidades de 1e-10 USD — verificado a mano contra
  // el costo real que muestra console.x.ai para la misma llamada. Se manda
  // de vuelta junto con el texto para poder mostrarle a Mario cuánto costó
  // cada sugerencia, sin tener que ir a mirar la consola de xAI.
  // El campo dejó de llegar en algún momento y por eso el contador decía
  // siempre cero. Se registran las claves de usage —solo los nombres, ningún
  // contenido— para que la próxima llamada diga cómo se llama ahora el dato,
  // en vez de andar probando a ciegas y gastando llamadas.
  if (!json.usage || json.usage.costInUsdTicks === undefined) {
    console.log('xAI no devolvió costInUsdTicks. Claves de usage:', json.usage ? Object.keys(json.usage).join(', ') : '(no vino usage)');
  }
  const costTicks = json.usage && json.usage.costInUsdTicks;
  const costUsd = costTicks !== undefined ? Number(costTicks) / 1e10 : null;
  return { text, costUsd };
}
...
async function suggestDescriptions(imageUrl, title, count = 3, lengthHint) {
  ...
  const { text, costUsd } = await askGrok([
    {
      role: 'user',
      content: [
...
async function suggestCountryIntro(countryName, count = 3) {
  ...
  const { text, costUsd } = await askGrok(prompt);
  ...
  return { suggestions: results.slice(0, count), costUsd };
}
...
async function translateToEnglish(text) {
  ...
  const { text: result, costUsd } = await askGrok(prompt);
  return { text: stripDecoration(result.split('\n')[0]), costUsd };
}
